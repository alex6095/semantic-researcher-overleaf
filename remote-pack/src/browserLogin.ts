...
export class BrowserLogin {
    static async login(context:vscode.ExtensionContext, request:BrowserLoginRequest): Promise<BrowserLoginResponse> {
        ...
        if (candidates.length===0) {
            throw new Error('Could not find Chrome, Edge, or Chromium. Set semantic-researcher-overleaf-remote-pack.browserPath and try again.');
        }
        ...
        if (browserContext===undefined) {
            throw new Error(`Could not open a browser for login: ${this.errorMessage(lastError)}`);
        }

        try {
            const page = browserContext.pages()[0] ?? await browserContext.newPage();
            await this.openProjectPage(page, request.serverUrl);
            const cookies = await this.waitForProjectCookies(browserContext, page, request.serverUrl, timeoutMs);
            return { cookies };
        } finally {
    ...
    private static async waitForProjectCookies(browserContext:BrowserContext, page:BrowserPage, serverUrl:string, timeoutMs:number) {
        ...
        throw new Error('Timed out waiting for Overleaf browser login.');
    }
    ...
    private static ensureGraphicalEnvironment() {
        if (process.platform==='linux' && !process.env.DISPLAY && !process.env.WAYLAND_DISPLAY) {
            throw new Error('Browser login requires a graphical browser. This local UI extension host looks headless; use Login with Cookies or run VS Code where a desktop browser is available.');
        }
    }

    private static getBrowserCandidates(configuredPath:string): BrowserCandidate[] {
        ...
        if (configuredPath.trim().length>0) {
            addCandidate('Configured browser', this.resolveExecutable(configuredPath.trim()));
        }
